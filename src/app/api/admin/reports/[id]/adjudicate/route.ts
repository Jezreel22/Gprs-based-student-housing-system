...
const RULE_BY_REPORT = { fake_property_listing: "fake_property_listing", spam_activity: "spam_activity" } as const;
...
export async function POST(req: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    if (body.status === "substantiated" && report.target_user_id && report.report_type in RULE_BY_REPORT) {
      await recordTrustEvent({ userId: report.target_user_id, ruleKey: RULE_BY_REPORT[report.report_type as keyof typeof RULE_BY_REPORT], sourceType: "report", sourceId: report.id, dedupeKey: `report:${report.id}`, actorId: officer.id, details: { report_type: report.report_type } });
    }
