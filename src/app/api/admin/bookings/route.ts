 ...
 *
 * Escrow-officer view of bookings that need oversight: those awaiting release
 * (pending_review), in flight (release_pending), or failed (release_failed).
...
export async function GET(req: NextRequest) {
  try {
    ...
    const rows = await db
      ...
      .from(bookingsTable)
      .where(
        inArray(bookingsTable.booking_status, ["pending_review", "release_pending", "release_failed"]),
      );
    ...
    const items = rows
      ...
      .map((b) => {
        ...
        return {
          ...
          property_address: propMap.get(b.property_id)?.address ?? null,
